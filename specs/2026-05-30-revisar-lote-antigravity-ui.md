...
- **ID 036 — Limpar concluídos:** Implementado e atende os critérios declarados
  (concluídos somem da visualização; itens não são apagados). Mecanismo atual:
  `bulkUpdateItems({ patch: { folderId: '' } })` — remove os concluídos da pasta
  (vão para "sem pasta"/Inbox). PONTO DE REVISÃO: o efeito colateral é mover os
  itens para fora da pasta, não apenas ocultá-los. Como atende os critérios do
  pedido e mudar o destino (ex.: arquivar) tem consequências de dados, mantido
  sem alteração e sinalizado para decisão do usuário.
...
- [ ] Rodar checks (type-check, lint).
- [ ] Capturar evidência visual dos itens visuais.
- [x] Decisão: nenhum código alterado (todos atendem os critérios).
...
- Decision: 036 mantido como está (move concluídos para fora da pasta) e
  sinalizado para revisão manual em vez de trocar por arquivar.
  Reason: Atende os critérios literais; mudar o destino tem consequência de
  organização de dados e seria uma decisão de produto do usuário.
  ADR needed: no.

## Files changed

- Nenhum. Tarefa de validação; nenhum item exigiu correção de código.
...
- Itens 031 (calendário `+x`), 035/036, 043, 045, 046, 047 validados por leitura
...
Commit/push da spec + evidências na branch `dev`. Revisão manual recomendada:
- ID 036: decidir destino dos concluídos ao "Limpar" (hoje vão para fora da
  pasta / Inbox). Se preferir mantê-los como concluídos sem sair da pasta, será
  necessário um sinalizador por item (mudança de schema, fora do escopo atual).
