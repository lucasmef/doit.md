...
function PropIcon({ name }: { name: 'status' | 'owner' | 'due' | 'tags' | 'edited' }) {
  const common = {
    viewBox: '0 0 24 24',
    fill: 'none',
    stroke: 'currentColor',
...
function PropsGrid({
  ...
  const dueLabel = formatReadableDate(item.dueDate)
  ...
  return (
    <dl className="mb-7 grid grid-cols-[110px_1fr] gap-x-4 gap-y-1 border-b border-navy-900/[0.04] pb-6 text-[13px]">
      ...
      <dd className="flex items-center gap-2 py-1 text-navy-900">
        <span className="inline-flex h-[22px] w-[22px] items-center justify-center rounded-full bg-[linear-gradient(135deg,#2F6BFF,#28C7B7)] text-[10px] font-bold text-white">
          LF
        </span>
        Local workspace <span className="font-mono text-[12px] text-navy-500">· 1 collaborator</span>
      </dd>
      ...
      <dd className="flex flex-wrap items-center gap-1.5 py-1">
        {item.tags.length === 0 ? (
          ...
          item.tags.map((tag, i) => {
            ...
            return (
              <span key={tag} className={`rounded px-2 py-0.5 font-mono text-[11px] ${tones[i % tones.length]}`}>
                #{tag}
              </span>
            )
          })
      ...
      <dt className="flex items-center gap-1.5 py-1 font-mono text-[11px] font-bold uppercase tracking-wider text-navy-500">
        <PropIcon name="edited" />
        edited
      </dt>
      <dd className="py-1 font-mono text-[12px] text-navy-500">
        {formatRelative(item.updatedAt)} · {wordCount} palavras
      </dd>
    </dl>
  )
}
