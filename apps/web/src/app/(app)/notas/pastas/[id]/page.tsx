...
export default function FolderDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function handleDelete() {
    ...
    if (typeof window !== 'undefined') window.location.href = '/notas/pastas'
  }
  ...
  return (
    <div className="mx-auto w-full max-w-6xl px-4 pb-28 pt-4 sm:px-6 lg:pb-8">
      <nav className="mb-4 flex flex-wrap items-center gap-1 font-mono text-[11px] text-navy-400">
        <Link href="/notas/pastas" className="rounded-full bg-white/42 px-2 py-1 hover:text-navy-700">
          Pastas
        </Link>
        {breadcrumb.map((f, idx) => (
          <span key={f.id} className="flex items-center gap-1">
            ...
            {idx === breadcrumb.length - 1 ? (
              ...
              <Link href={`/notas/pastas/${f.id}`} className="rounded-full bg-white/42 px-2 py-1 hover:text-navy-700">
                {f.name}
              </Link>
            )}
          </span>
