...
export async function DELETE(_req: NextRequest, { params }: Params) {
  try {
    ...
    const allFolders = (await FolderModel.find({ userId }).lean()) as { _id: string; parentId?: string }[]
    ...
    for (const folderId of descendants) {
      ...
      await ItemModel.updateMany({ userId, folderId }, { folderId: null, updatedAt: new Date().toISOString() })
    }
