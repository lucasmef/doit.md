...
export function CalendarEventCapture() {
  ...
  return (
    <div
      className="fixed inset-0 isolate z-[220] flex items-end justify-center overflow-hidden bg-navy-900/35 p-0 backdrop-blur-[2px] sm:items-center sm:p-4"
      role="dialog"
      aria-modal="true"
      onClick={(clickEvent) => {
      ...
      {...swipeHandlers}
    >
      {/* Aurora só no modo expandido; no compacto o card fica esbranquiçado/opaco (ID 030). */}
      {isExpanded ? (
        <div
          aria-hidden="true"
      ...
      >
        ...
        <div className={`${isExpanded ? 'flex' : 'hidden sm:flex'} shrink-0 items-center justify-between gap-2 border-t border-navy-900/[0.07] bg-white/[0.62] px-5 py-3.5 pb-[calc(0.875rem+env(safe-area-inset-bottom))]`}>
